...
execute 'app_extract' do
  user 'root'
  command "unzip -o #{node[:jboss][:tmp]}/#{node[:jboss][:app_zip]} -d #{node[:jboss][:jboss_home]}/#{node[:jboss][:jboss_dir]}/#{node[:jboss][:deploy_app]}"
  action :run
  not_if { ::File.directory?("#{node[:jboss][:jboss_home]}/#{node[:jboss][:jboss_dir]}/#{node[:jboss][:deploy_app]}/#{node[:jboss][:app]}")}
end
...
template "#{node[:jboss][:jboss_home]}/#{node[:jboss][:jboss_dir]}/#{node[:jboss][:deploy_app]}/#{node[:jboss][:app]}/hudson.xml" do
  source "hudson.erb"
  mode "0755"
  owner "#{node[:jboss][:jboss_user]}"
  group "#{node[:jboss][:jboss_user]}"
  variables({
     :engine => data_bag_item('hudson', 'hudson')['cumulogic-app']['services']['framework']['engine']
})
end
